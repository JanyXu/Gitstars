...
import cn.gitstars.gitstars.http.ServerUri;
...
public class CommonUtil {
    ...
    public static void hideInputMethod(View view) {
        if (view == null)
            return;
        // 隐藏键盘
        Context context = ProApplication.getmContext();
    ...
    public static String getShareUrl(String title) {
        return ServerUri.GIT_STARS_SHARE + StringUtil.urlEncode(title);
    }
    ...
    public static int getVersion() {
        ...
        if (mPackageManager == null) {
            mPackageManager = ProApplication.getmContext()
                    .getPackageManager();
        }
        // getPackageName()是你当前类的包名，0代表是获取版本信息
        PackageInfo packInfo;
        try {
            packInfo = mPackageManager.getPackageInfo(ProApplication.getmContext().getPackageName(), 0);
